...
**矩阵定位**: 副作用语义维度的 T2 格 + 断点恢复维度的 T2 格。**答案: 不会双重执行。** `ledger_store.go:104-107` 的 `hasOrphanedAcquired` 检测到事件流中存在 `ledger_acquired` 但无匹配 `ledger_committed`（崩溃窗口），返回 `WaitOtherWorker`。如果该 orphaned 记录超时，清理后重新 `Acquire`。如果工具实际已执行但未 Commit，下一次 Acquire 会发现无 committed 记录并允许重新执行——**这是 at-most-once 的已知边界**：Acquire 后 Commit 前的崩溃可能导致重执行。但 `atomic_commit.go:52-94` 的原子序列（先 `ledger_acquired` → 执行 → `ledger_committed`）确保了事件流可检测此窗口。**NOT PROVEN**: `ToolNodeAdapter.runNodeExecute` 路径（非 `DefaultAtomicCommit` 路径）是否也写 `ledger_acquired` 事件——需确认 `node_adapter.go` 的完整事件写入序列。
